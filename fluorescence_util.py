...
class EEMF7000():
    ...
    def remove_self_reflection_and_scattering_from_eem(
            ...
            *, 
            remove_first_order,  # ここで1次散乱除去の有無を指定
            inplace=False, verbose=False
            ):
        eem_df = self.eem_df
    ...
    def remove_scatter_regions(self, *, inplace=False):
        """
        EEM図上で以下の散乱領域を除去：
        - 1次反射線より左側（emission < excitation）
        - 2次散乱線より右側（emission > 2 * excitation）

        前提：EEMは (excitation [縦], emission [横])
        """
        if not inplace:
            eem_df = self.eem_df.copy()
        else:
            eem_df = self.eem_df

        for ex in self.ex_bands:
            for em in self.em_bands:
                if em < ex or em > 2 * ex:
                    eem_df.loc[ex, em] = np.nan

        if inplace:
            self.eem_df = eem_df
        return eem_df
    
    def remove_scatter_regions_lowerright(self, *, inplace=False):
        """
        EEM図上で2次散乱線より右側（emission > 2 * excitation）のみを除去。
