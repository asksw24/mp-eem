...
from scipy.spatial.distance import squareform # 距離行列の形式変換用

# --- あなたの実際のデータに置き換えてください ---
# eem_array_normalized と sample_name が既にロードされていると仮定します。

# 仮のeem_array_normalized と sample_name を生成 (あなたのデータに合わせてください)
# 形状: (9, 71, 71) に合わせます。
...
# 形状は (9, 71*71) = (9, 5041) となります。
flat_eem_features = eem_array_imputed_for_flat.reshape(eem_array_imputed_for_flat.shape[0], -1)

print(f"平坦化されたEEMデータの形状: {flat_eem_features.shape}")
print("--- ステップ2: EEMデータの平坦化が完了しました ---")

# %% [markdown]
# ## コサイン類似度（距離）の計算

# %%
# --- ステップ3: コサイン類似度（距離）の計算 ---

print("\n--- ステップ3: コサイン類似度行列を計算します ---")

# sklearn.metrics.pairwise.cosine_similarity を使用
# 結果は類似度行列です (値が大きいほど似ている)
cosine_sim_matrix = cosine_similarity(flat_eem_features)

print("コサイン類似度行列の形状:", cosine_sim_matrix.shape)
print("コサイン類似度行列（一部）:\n", np.round(cosine_sim_matrix[:3, :3], 3)) # 先頭3x3を表示

# 階層的クラスタリングには距離行列（非類似度）が必要なので変換
cosine_dist_matrix = 1 - cosine_sim_matrix

print("\nコサイン距離行列の形状:", cosine_dist_matrix.shape)
print("コサイン距離行列（一部）:\n", np.round(cosine_dist_matrix[:3, :3], 3)) # 先頭3x3を表示
print("--- ステップ3: コサイン類似度（距離）の計算が完了しました ---")
...
print("\n--- ステップ3: 階層的クラスタリングを開始します ---")

# linkage関数に平坦化された特徴量 flat_eem_features を直接渡し、
# 距離計算方法として metric='cosine' を指定します。
# これにより、内部でコサイン距離が計算され、対角ゼロの問題も適切に処理されます。
linkage_matrix = linkage(flat_eem_features, method='average', metric='cosine')

print("--- ステップ3: 階層的クラスタリングが完了しました ---")

# %% [markdown]
# ## デンドログラム作成

# %%
# --- ステップ4: デンドログラムのプロットと保存 ---

print("\n--- ステップ4: デンドログラムをプロットします ---")

plt.figure(figsize=(12, 8))
dendrogram(
    linkage_matrix,
    labels=sample_name,     # サンプル名をラベルとして使用
    leaf_rotation=90,       # ラベルを縦向きにする
    leaf_font_size=10,      # ラベルのフォントサイズ
    color_threshold=0.5,    # クラスタリングの閾値を色で示す（この距離以下で同じ色）
    above_threshold_color='grey' # 閾値以上の線はグレーにする
)
plt.title('Microplastic EEM Dendrogram (Direct Cosine Distance, Ward Linkage)', fontsize=16)
plt.xlabel('Microplastic Sample', fontsize=12)
plt.ylabel('Cosine Distance', fontsize=12)
plt.grid(axis='y', linestyle='--', alpha=0.7)
plt.tight_layout() # レイアウトを自動調整
plt.show()
# output_filename = 'microplastic_eem_direct_dendrogram.png'
# plt.savefig(output_filename)
# plt.close() # メモリ解放のためプロットを閉じる

print(f"--- ステップ4: デンドログラムは '{output_filename}' として保存されました ---")
